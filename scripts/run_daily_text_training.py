...
from fly_window.publishing.runtime import RuntimeConfig, RuntimeState, health_alerts
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run one durable no-LLM Fly Tweets cycle. The MaleCNS text policy learns "
            "from settled engagement and generates exactly 10 new tweets. Publication "
            "times are intentionally absent here: Google Apps Script owns scheduling."
        )
    )
...
def main() -> None:
    ...
    training_examples = training_examples_from_rows(rows, vocabulary=vocabulary)
    training_applied = bool(training_examples)
    ...
    def write_checkpoint(updated_policy, written_checkpoint_id, manifest):
        ...
        complete_manifest["parent_checkpoint_id"] = current_checkpoint_id
        ...
        if manifest_path.exists() or checkpoint_path.exists():
            ...
            if any(existing.get(field) != complete_manifest.get(field) for field in stable_fields):
        ...
        payload["daily_manifest"] = complete_manifest
    ...
    if training_applied:
        result = run_daily_update(
            ...
            generation_count=DAILY_GENERATION_COUNT,
        )
        active_checkpoint_id = result.checkpoint_id
        ...
        generated = generate_batch(
            ...
            git_sha=git_sha,
            batch_id=batch_id,
            count=DAILY_GENERATION_COUNT,
        )
        generated_rows = [
            generated_tweet_row(tweet, generated_at=now)
            for tweet in generated
        ]
        if len(generated_rows) != DAILY_GENERATION_COUNT:
            raise RuntimeError("generation-only cycle did not produce exactly 10 tweets")
        _append_outbox(args.outbox_jsonl, generated_rows)
        active_checkpoint_id = current_checkpoint_id
        consumed_rows = 0
    ...
    next_state = replace(
        ...
        last_training_at=now if training_applied else runtime_state.last_training_at,
        last_generation_at=now,
        current_checkpoint=active_checkpoint_id,
    )
